...
export async function createProduct(req, res) {
    try {
        const { title, description, price, category, sellerId, condition, is_auction } = req.body;
        ...
        if (!title ||
            ...
            !sellerId) {
            return res.status(400).json({
                message: "Missing required fields: title, description, price, category, sellerId",
            });
        }
...
export async function updateProductStatus(req, res) {
    try {
        ...
        const updatedProduct = await Listing.findByIdAndUpdate(productId, {
            status,
            $set: { updatedAt: new Date() }
        }, { new: true }).populate("categoryId", ["name", "description"]);
        if (!updatedProduct) {
...
export async function updateProduct(req, res) {
    try {
        const { productId, title, description, price, category, imageUrl, condition, is_auction } = req.body;
        if (!productId) {
            return res
                .status(404)
        ...
        const updateData = {};
        if (title)
            updateData.name = title; // Map title to name
        if (description)
            updateData.description = description;
        if (price)
            updateData.price = Number(price);
        if (imageUrl)
            updateData.imageId = imageUrl; // Map imageUrl to imageId
        if (condition)
        ...
        const updatedProduct = await Listing.findByIdAndUpdate(productId, updateData, { new: true }).populate("categoryId", ["name", "description"]);
        if (!updatedProduct) {
            return res.status(404).json({ message: "Product not found" });
        }
